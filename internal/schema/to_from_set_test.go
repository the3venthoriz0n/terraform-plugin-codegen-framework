...
l, d := basetypes.NewSetValueFrom(ctx, types.BoolType, elems)

diags.Append(d...)

if diags.HasError() {
return ExampleValue{
types.SetUnknown(types.BoolType),
}, diags
}
...
`),
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			toFromSet := NewToFromSet(testCase.name, testCase.assocExtType, testCase.elemTypeType, testCase.elemTypeValue, testCase.elemFrom)

			got, err := toFromSet.renderFrom()
...
func TestToFromSet_renderTo(t *testing.T) {
	t.Parallel()
